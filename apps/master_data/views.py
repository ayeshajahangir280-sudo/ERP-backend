...
# Items are different from ordinary historical master data: removing one must
# also remove every movement and adjustment that could make it reappear.
RawMaterialViewSet.perform_destroy = lambda self, instance: hard_delete_instance(instance)
FinishedProductViewSet.perform_destroy = lambda self, instance: hard_delete_instance(instance)
